...
export default async function DashboardPage() {
  ...
  const { data: decision_speed_logs, error: decisionSpeedError } = await supabaseClient
  ...
  const { data: wlan_tracking_log, error: wlanTrackingError} = await supabaseClient
  ...
  .from("tracking_log")
  .select('id, ip_address, ip_address, device_type, browser, os, country, referrer, session_id, timestamp');
  if (trackingError) {
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 px-4">
        <Card>
            <CardHeader className="relative">
              <CardDescription>Sessions insgesamt</CardDescription>
            ...
            <CardHeader className="relative">
              ...
              <CardTitle className="text-2xl font-semibold tabular-nums">
                {session_time_logs && session_time_logs.length > 0
                  ? `${Math.round(
                    session_time_logs.reduce((acc, curr) => acc + curr.time_on_page_ms, 0) / session_time_logs.length / 1000
                    )} Sek.`
                  : "0 Sek."}
              </CardTitle>
            </CardHeader>
            ...
            <CardHeader className="relative">
              ...
              <CardTitle className="text-2xl font-semibold tabular-nums">
                {decision_speed_logs && decision_speed_logs.length > 0
                  ? `${Math.round(
                    decision_speed_logs.reduce((acc, curr) => acc + curr.time_to_submit_ms, 0) / decision_speed_logs.length / 1000
                    )} Sek.`
                  : "0 Sek."}
              </CardTitle>
            </CardHeader>
  ...
  <CardHeader className="relative">
    ...
    <CardTitle className="text-2xl font-semibold tabular-nums">
      {click_log
        ? click_log.filter((click) => click.text === 'Jetzt teilnehmen').length
        : 0}
    </CardTitle>
  </CardHeader>
  <CardFooter className="text-muted-foreground">
